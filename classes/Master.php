...
Class Master extends DBConnection {
	...
	function save_cargo(){
		if(empty($_POST['id'])){
			$pref = "ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890";
			$pref2 = "CFL". substr(str_shuffle($pref), 0 , 7);
			$pref3 = strtoupper($pref2);
			$code = sprintf("%'.05d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM `cargo_list` where ref_code = '{$pref3}{$code}'")->num_rows;
				if($check > 0){
		...
		if(empty($id)){
				$save_track = $this->add_track($cid,"Pending", $receiver_name . ", your package has been received by our agent");
		 }
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
